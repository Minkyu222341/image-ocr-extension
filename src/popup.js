        ...
        canvas.toBlob(
          (compressedBlob) => {
            if (compressedBlob.size <= maxSizeKB * 1024 || quality <= 0.1) {
              console.log(
                `이미지 압축 완료: ${Math.round(imageBlob.size / 1024)}KB → ${Math.round(compressedBlob.size / 1024)}KB`
              );
...
async function performOCR(imageBlob, language) {
  try {
    ...
    // 다중 엔진 시도로 복합 이미지 인식률 향상
    const apiKey = "K87899142388957"; // 무료 API 키 (월 25,000회 제한)
    const engines = [2, 1]; // 엔진 우선순위: 2(표준) → 1(기본) - 엔진 3 제거
    ...
    for (let i = 0; i < engines.length; i++) {
      ...
      try {
        console.log(`OCR 엔진 ${engine} 시도 중...`);
        updateProgress(`OCR 엔진 ${engine} 처리 중...`, 40 + i * 15);

        const formData = new FormData();
        formData.append("file", processedBlob, "image.jpg");
        formData.append("apikey", apiKey);
        formData.append("language", getOCRLanguageCode(language, engine));
        ...
        // 엔진별 최적화 옵션
        if (engine === 3) {
          formData.append("isTable", "true"); // 복잡한 레이아웃
        } else {
          formData.append("isTable", "false"); // 일반 텍스트
        }
        ...
        if (!response.ok) {
          console.warn(`엔진 ${engine} API 오류: ${response.status}`);
          updateProgress(
            `엔진 ${engine} 실패, 다음 엔진 시도 중...`,
            40 + i * 15
          );
          continue;
        }
        ...
        if (result.IsErroredOnProcessing) {
          console.warn(`엔진 ${engine} 처리 오류:`, result.ErrorMessage);
          updateProgress(
            `엔진 ${engine} 처리 실패, 다음 엔진 시도 중...`,
            40 + i * 15
          );
          continue;
        }
        ...
        if (result.ParsedResults && result.ParsedResults.length > 0) {
          ...
          console.log(`엔진 ${engine} 결과: ${textLength}자 추출`);

          // 더 많은 텍스트를 추출한 엔진의 결과 선택
          if (textLength > bestLength) {
            bestResult = extractedText;
            bestLength = textLength;
            console.log(`엔진 ${engine}에서 더 나은 결과 발견`);
          }

          // 충분한 텍스트가 추출되면 조기 종료
          if (textLength > 100) {
            console.log(`엔진 ${engine}에서 충분한 텍스트 추출 완료`);
            break;
          }
        }
      } catch (engineError) {
        console.warn(`엔진 ${engine} 실패:`, engineError.message);
        continue;
      }
    }

    return bestResult || "텍스트를 찾을 수 없습니다.";
  } catch (error) {
    console.error("OCR 처리 오류:", error);
...
// OCR 언어 코드 변환 (엔진별 최적화)
function getOCRLanguageCode(language, engine = 2) {
  // 엔진 3은 언어 자동 감지 사용 (더 정확함)
  if (engine === 3) {
    return ""; // 빈 문자열 = 자동 언어 감지
  }
...
function enhanceImageForOCR(imageData) {
  ...
  // 2. 밝기 조정 (Brightness Adjustment)
  const brightnessFactor = 10; // 밝기 증가값
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    // 3. 선명도 향상을 위한 그레이스케일 변환 (선택적)
    const shouldConvertToGray = (r + g + b) / 3 < 180; // 어두운 텍스트만
    if (shouldConvertToGray) {
      const gray = Math.round(0.299 * r + 0.587 * g + 0.114 * b);
      // 텍스트를 더 진하게 만들기
      const enhancedGray =
        gray < 128 ? Math.max(0, gray - 30) : Math.min(255, gray + 30);
      r = g = b = enhancedGray;
    }
...
function showResult(text, confidence) {
  ...
  if (text && text.trim()) {
    ...
    if (confidence < 70) {
      showMessage(
        `텍스트 인식 신뢰도: ${Math.round(confidence)}%. 복잡한 폰트나 색상의 경우 수동 수정이 필요할 수 있습니다.`,
        "error"
      );
    } else {
      showMessage(
        `텍스트가 자동으로 클립보드에 복사되었습니다! (신뢰도: ${Math.round(confidence)}%) 결과를 확인 후 필요시 수정해주세요.`,
        "success"
      );
    }
  } else {
...
function copyToClipboard(auto = false, event = null) {
  ...
  // Chrome Extension에서 더 안전한 클립보드 복사 방법
  try {
    // 먼저 텍스트 영역에 포커스
    resultText.focus();
    resultText.select();
...
// 폴백 복사 함수
function fallbackCopy(text, auto, event) {
  try {
    const resultText = document.getElementById("resultText");
    resultText.focus();
    resultText.select();
    ...
    if (successful) {
      ...
      if (!auto) {
        showMessage("텍스트가 선택되었습니다. Ctrl+C로 복사해주세요.", "error");
      }
    }
  } catch (e) {
    console.error("폴백 복사 실패:", e);
    if (!auto) {
      showMessage("텍스트가 선택되었습니다. Ctrl+C로 복사해주세요.", "error");
    }
  }
...
// 복사 버튼 상태 업데이트
function updateCopyButton(event) {
